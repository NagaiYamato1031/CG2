...
const wchar_t WinApp::kWindowClassName[] = L"LE2A_12_ナガイ_ヤマト";
		...
		if (app && app->GetSizeChangeMode() == WinApp::SizeChangeMode::kFixedAspect) {
			...
			case WMSZ_BOTTOM:
				rect->right = rect->left +
					LONG((rect->bottom - rect->top) * aspectRatio);
				break;
			case WMSZ_TOPLEFT:
...
void WinApp::CreateGameWindow(const wchar_t* title, UINT windowStyle, int32_t clientWidth, int32_t clientHeight) {
	...
	// ウィンドウクラスの設定
	//wndClass_.cbSize = sizeof(WNDCLASSEX);			// ウィンドウプロシージャ
	wndClass_.lpfnWndProc = (WNDPROC)WindowProc;	// ウィンドウクラス名
	wndClass_.lpszClassName = kWindowClassName;		// ウィンドウハンドル
